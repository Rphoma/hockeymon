...
# players = [x[:-9] for x in url_list.keys() if poke_gen in x]
...
import urllib
import pandas as pd
url_list = pd.read_csv('https://docs.google.com/spreadsheets/d/1boGNk714eJVhPk9kZDB-BMF4diXHw5cpM3yMlIgtmRo/export?format=csv&gid=0').set_index('File Name').to_dict()['URL']
players = [x[:-9] for x in url_list.keys() if poke_gen in x]
default_ix = players.index('Jason Robertson')
player = st.selectbox('Select a player:', players, index=default_ix)
card_loc = url_list[f"{player}-{poke_gen}.png"]
